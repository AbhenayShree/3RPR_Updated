...
def collect_experience(replay_buffer, desired_state, N, Q, R, num_episodes=100, max_steps=200, step_size=0.01, lambda_torque=0.01):
    for _ in range(num_episodes):
        state = np.random.uniform([-10,-10,0,-1,-1,-1], [10,10,2*np.pi,1,1,1])  # Random initial state
        x_vals, u_vals, t_vals = mpc(state, desired_state, max_steps, N, step_size, Q, R)
        for step in range(len(x_vals)):
            action = u_vals[step,:]
            next_state = x_vals[step,:]
            reward = compute_reward(next_state, action, desired_state, lambda_torque = lambda_torque)
            done = np.linalg.norm(next_state - desired_state[:len(Q)]) < 1e-2
...
def train_loop(actor_model, critic_model, target_actor, target_critic, replay_buffer, N, Q, R,
    ...
    for iteration in range(num_iterations):
        ...
        collect_experience(replay_buffer, desired_state, N, Q, R,
...
actor_model.save("rlmpc_actor_model.keras", save_format='keras')
critic_model.save("rlmpc_critic_model.keras", save_format='keras')
target_actor.save("rlmpc_target_actor.keras", save_format='keras')
target_critic.save("rlmpc_target_critic.keras", save_format='keras')

print("Models saved successfully in `.keras` format!")

# Load models
actor_model = load_model("rlmpc_actor_model.keras")
critic_model = load_model("rlmpc_critic_model.keras")
target_actor = load_model("rlmpc_target_actor.keras")
target_critic = load_model("rlmpc_target_critic.keras")
...
def plot_control_results(history):
    ...
    plt.tight_layout()
    plt.savefig(f'rlmpc_result.png', bbox_inches='tight', transparent=True, dpi=1200)
...
initial_state = (0.01, 0.02, math.radians(15), 0, 0, 0)
history = compute_control_forces(actor_model, initial_state, dt=0.01, max_steps = 200)
